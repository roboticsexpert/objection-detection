...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            newHeight = int(img_size / width * height)

            newWidth = img_size

            print(
                f' Source was originaly {width}x{height} pixel with {fps} fps and we are scaling them to : {newWidth}x{newHeight} pixels')

            cap.set(cv2.CAP_PROP_FRAME_WIDTH, newWidth)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, newHeight)
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            if time.time() - last_fps_calculated_at >= 1:
                fps = self.frame_id / (time.time() - last_fps_calculated_at)
                self.frame_id = 0
                last_fps_calculated_at = time.time()
