...
def detect(save_img=False):
    ...
    # Ininitite viewer
    drawer = Drawer(dataset, webcam, names, colors)
    drawer.start()
    ...
    #     print(f'{s}Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS, Create image time: ({(1E3 * (t4 - t3)):.1f}ms)')
...
class Drawer(Thread):
    def __init__(self, dataset, webcam, names, colors):
    ...
    def run(self):
        ...
        while True:
            ...
            while last_frame_id == self.dataset.frame_id:
                time.sleep(0.5 / fps)

            frame = self.dataset.imgs[0]
            last_frame_id = self.dataset.frame_id
        ...
        while True:
            cv2.imshow(str("temp"), cv2.resize(self.dataset.imgs[0], (800, 600)))
            time.sleep(1 / 30)
            # for i, det in enumerate(self.pred):  # detections per image
            #    startedAt=time.time()
            #    if self.webcam:  # batch_size >= 1
            #        p, s, im0, frame = path[i], '%g: ' % i, im0s[i], self.dataset.count
            #    else:
            ...
            # counter+=1
            # if counter%10 ==0:
            #     print(f"create image({(1E3 * (tImage- startedAt )):.1f}ms) show ({(1E3 * (tShow- tImage )):.1f}ms)")

            # block for a moment

            # if self.hasImage:
            #     cv2.imshow("Output view",self.image)
            #     if cv2.waitKey(20) == ord('q'):  # q to quit
            #         print("Stop visual !!!!!")
            # else:
            #     time.sleep(1)
